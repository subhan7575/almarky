...
const AddressBook: React.FC = () => {
  const { user, addresses, deleteAddress, setDefaultAddress } = useAuth();
  ...
  const handleSetDefault = async (addressId: string) => {
    if (user) await setDefaultAddress(user.uid, addressId);
  };
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex justify-between items-center">
        ...
        <button 
          ...
          className="text-[9px] font-black text-blue-600 uppercase border border-blue-100 px-3 py-1.5 rounded-lg hover:bg-blue-50 transition active:scale-95"
        >
          + Add New
        </button>
      </div>
      ...
      {addresses.length > 0 ? (
        <div className="space-y-4">
          {addresses.map(addr => (
            <div key={addr.id} className="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm relative group">
              ...
              <div className="absolute top-5 right-5 flex space-x-2 opacity-0 group-hover:opacity-100 transition-opacity">
                {!addr.isDefault && (
                  <button onClick={() => handleSetDefault(addr.id)} className="text-slate-300 hover:text-emerald-600" title="Set as Default">
                     <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7"></path></svg>
                  </button>
                )}
                <button onClick={() => handleEdit(addr)} className="text-slate-300 hover:text-blue-600" title="Edit Address"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg></button>
                <button onClick={() => handleDelete(addr.id)} className="text-slate-300 hover:text-rose-600" title="Delete Address"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg></button>
              </div>
            </div>
          ))}
        </div>
         ...
         <div className="py-20 text-center bg-slate-50 rounded-[2.5rem] border-2 border-dashed border-slate-200">
          <p className="text-xs font-bold text-slate-400 uppercase">You have no saved addresses.</p>
        </div>
      )}
